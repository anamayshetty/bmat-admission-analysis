...
The purpose of our analysis is to get model which for a given BMAT score, will give us a probabilty of being accepted. Below, are two potential options:
...
Now let's take a look at the logistic model: brace youself:
...
Let's start with the linear model: I've put the 

```{r Prior Predictive Modelling for Linear Regression}

cross_df(list(x = seq(-3, 3, 0.1), a = rnorm(20, 0.5, 0.07), b = exp(rnorm(20, log(1/6), 0.5)))) %>%
  mutate(y = a + b * x) %>%
  unite(col = "ab", a, b) %>%
